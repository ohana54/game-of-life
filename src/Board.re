...
let initialState = {
  running: false,
  board: BoardData.newBoard(boardSize)
};
...
let reducer = (state, action) => {
  ...
  | Tick => {
    ...state,
    board: BoardData.nextBoard(state.board)
  }
  | Start => {
    ...state,
    running: true
...
let createCell = (index, cell) => {
  open BoardData;
  let bgColor = switch cell {
    | Dead => "white"
    | Alive => "black"
  };
  let style = ReactDOMRe.Style.make(~backgroundColor=bgColor, ());
  <td style={style} key={string_of_int(index)}></td>
};
...
let make = () => {
  ...
  let delay = switch (state.running) {
    | true => Some(75)
    | false => None
  };
  ...
  <div
    style={ReactDOMRe.Style.make(~display="flex", ~alignItems="center", ~justifyContent="space-between", ~flexDirection="column", ())}>
    <table>
      <tbody>
      {
