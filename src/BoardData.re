...
let getNewCell = (board, rowIndex, colIndex) => {
  let numOfLiveNeighbors = getNeighbors(board, rowIndex, colIndex) -> Belt.Array.keep(isAlive) -> Belt.Array.size
...
let createRowData = (rowSize, _) => {
  Belt.Array.makeBy(rowSize, (_) => {
    Random.int(10) > 7 ? Alive : Dead
  })
}
